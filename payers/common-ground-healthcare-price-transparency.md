...
**August 2026 Status:** No machine-readable files for the August 2026 cycle were found. Common Ground Healthcare is owned by CareSource; no separate files were found.
...
**Overall Assessment:** While the data is available, accessing the files can be complex to find as they are nested in multiple layers. This requires extra manual processing in order to even extract rates of the same type & category. The data seems to be duplicated in places which could be due to an interpretation of the CMS regulations but there is room for improvement in accessibility. Rescored to 1/5 in August 2026 because no August 2026 files were found; Common Ground Healthcare is owned by CareSource and does not publish separate files.
